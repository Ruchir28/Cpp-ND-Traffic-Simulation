...
void TrafficLight::simulate()
{
    ...
    std::thread myThread(&TrafficLight::getCurrentPhase,this);
...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::mt19937 eng(rd());
    std::uniform_int_distribution<> distr(4000, 6000); // between 4000 to 6000 ms, i.e 4 to 6 seconds

    auto prev = std::chrono::high_resolution_clock::now();
    while(true) {
        auto current = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(current - prev).count();
